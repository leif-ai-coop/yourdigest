...
| P0-2 | Kein Endpoint erzwingt Auth | ⏪ ZURÜCKGEROLLT | `auth=[Depends(get_current_user)]` aktiviert → App tot ("Not authenticated", leere Seiten). Ursache: frontend-nginx leitet die `X-authentik-*` Header beim `/api/`-Proxy **nicht** an die API weiter (NPM `7.conf` setzt sie korrekt, aber der 2. Hop verliert sie). Wieder rausgenommen. **TODO vor Re-Aktivierung:** in `frontend/nginx.conf` `proxy_set_header X-authentik-* $http_x_authentik_*;` ergänzen, end-to-end testen, *dann* erst Dependency setzen. Port 8000 ist nicht öffentlich → kein akutes Loch |
